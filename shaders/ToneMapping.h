...
vec3 toneMapWithWhitePointAdjustment(vec3 v, vec3 whitePoint) {
    ...
    vec3 white_scale = vec3(1.0) / toneMap_Uchimura(whitePoint);
    return curr * white_scale;
}

float brightnessMultiplier = 3.0;

vec3 toneMap(vec3 color) {
#ifdef APPLY_COLOR_TEMPERATURE
    vec3 whitePoint = vec3(colorTemperatureToRGB(6500.0));
#else
