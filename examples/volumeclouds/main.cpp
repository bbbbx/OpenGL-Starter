...
void VolumeCloudsDemo::Initialize(int argc, char** argv, const char* title) {
  ...
  std::vector<GLint> lineRange( 2 );
  glGetIntegerv( GL_ALIASED_LINE_WIDTH_RANGE, lineRange.data() );
  std::cout << "GL_ALIASED_LINE_WIDTH_RANGE: " << lineRange[0] << ", " << lineRange[1] << std::endl;
  glLineWidth( lineRange[1] );
